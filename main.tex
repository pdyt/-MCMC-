...
% 标题排版（关键调整：让大标题往下，缩小与摘要的距离）
\makeatletter
\renewcommand{\@maketitle}{%
  \vspace*{-0.5em} % 减少顶部负间距（原-2.5em → 改为-0.5em，让标题往下）
  \centering
  {\Large \bfseries \@title \par}%
  \vskip -2em % 增加标题到正文的间距（原0em → 改为1.5em，缩小与摘要的距离）
  {\large
    \lineskip 0em%
    \begin{tabular}[t]{c}%
      \@author
...
实证结果表明，比特币对数波动率存在\textbf{弱滞后相关性}，其波动过程呈现出显著的不确定性特征，印证了 SV 模型对加密资产波动规律的刻画能力。本研究不仅证实了 \textbf{SV-MCMC 方法}在比特币收益波动率估计中的\textbf{适用性与优越性}，更为投资者制定科学的风险管理策略、监管机构完善加密货币市场监管框架提供了重要的\textbf{理论支撑与实证依据}。未来可进一步拓展至含\textbf{跳跃项}、\textbf{长期记忆性}或\textbf{多变量结构}的复杂 SV 模型，以更全面地捕捉加密货币市场的动态波动特征。
